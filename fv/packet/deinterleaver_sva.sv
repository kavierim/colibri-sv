...
// Translated from fv/packet/deinterleaver.psl.
// Ready-low assumes are formal-only; see header_add_sva.sv.

`timescale 1ns/1ps

module deinterleaver_sva #(
  ...
  input logic [colibri_utils::downto_width(colibri_utils::log2ceil(int'(g_NUM_OUTPUTS)))-1:0] snk_channel_i,
  ...
  pkt_t srcs_arr [0:g_NUM_OUTPUTS-1];
  ...
  always_ff @(posedge clk_i) begin : proc_snk_pkt
    for (int i = 0; i < g_NUM_OUTPUTS; i++) begin
      if (reset_i) begin
        srcs_arr[i] <= PKT_IDLE;
      end else if (snk_channel_i == c_CH_W'(i) && snk_valid_i && snk_ready_o) begin
  ...
  for (genvar gi = 0; gi < g_NUM_OUTPUTS; gi++) begin : gen_in
    a_no_sop_in_multi: assume property (@(posedge clk_i)
      (srcs_arr[gi] == PKT_MULTI && snk_channel_i == c_CH_W'(gi)) |-> !snk_sop_i);

    a_no_valid_outside: assume property (@(posedge clk_i)
      (!snk_sop_i && srcs_arr[gi] != PKT_MULTI && snk_channel_i == c_CH_W'(gi))
      |-> !snk_valid_i);
...
`endif
  ...
  for (genvar go = 0; go < g_NUM_OUTPUTS; go++) begin : gen_out
    a_valid_out_sop: assert property (@(posedge clk_i)
      (!reset_i && snks_arr[go] == PKT_IDLE && src_ready_i[go] && src_valid_o[go])
      |-> src_sop_o[go]);

    a_valid_out_multi: assert property (@(posedge clk_i)
      (!reset_i && snks_arr[go] == PKT_MULTI && src_ready_i[go] && src_valid_o[go])
      |-> !src_sop_o[go]);

    c_valid_out_single: cover property (@(posedge clk_i)
      !reset_i && src_ready_i[go] && src_valid_o[go] && src_sop_o[go] && src_eop_o[go]);
  end

  a_only_one_snk_active: assert property (@(posedge clk_i)
    !reset_i |-> $onehot0(src_valid_o));

endmodule

bind deinterleaver deinterleaver_sva #(
