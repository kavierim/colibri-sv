...
// Packet-shape checks for be_add_lead, translated from fv/misc/be_add_lead.psl.

`timescale 1ns/1ps

module be_add_lead_sva #(
  // PSL pins snk_data_i to 0 and snk_lead_i to 32'hAAAAAAAA to simplify
  ...
  assume property (@(posedge clk_i) g_FORMAL_SIMPLIFY |-> (snk_lead_i == 32'hAAAAAAAA));
  assume property (@(posedge clk_i) g_FORMAL_SIMPLIFY |-> (snk_data_i == '0));
  assume property (@(posedge clk_i) int'(shl_i) <= c_WORD_BYTES);

endmodule

bind be_add_lead be_add_lead_sva be_add_lead_sva_i (.*);
