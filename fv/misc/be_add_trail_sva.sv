...
// Packet-shape checks for be_add_trail, translated from fv/misc/be_add_trail.psl.

`timescale 1ns/1ps

module be_add_trail_sva #(
  // PSL pins snk_data_i to 0 and snk_trail_i to 32'hAAAAAAAA to simplify
  // formal search. The VHDL scenario drives a real payload, so that pair
  // stays off unless a formal run sets this parameter.
  ...
  assume property (@(posedge clk_i) g_FORMAL_SIMPLIFY |-> (snk_trail_i == 32'hAAAAAAAA));
  assume property (@(posedge clk_i) g_FORMAL_SIMPLIFY |-> (snk_data_i == '0));
  assume property (@(posedge clk_i) int'(shl_i) <= c_WORD_BYTES);

endmodule

bind be_add_trail be_add_trail_sva be_add_trail_sva_i (.*);
