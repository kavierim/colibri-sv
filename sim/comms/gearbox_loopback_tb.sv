...
// Scenario from gearbox_loopback_tb.vhdl. run.py sweeps g_A_WIDTH in {16,32}
// and g_B_WIDTH in 1..128. Both sides stall with probability 1/2.

`timescale 1ns/1ps

module gearbox_loop_case #(
    ...
    if (reset) begin
      ...
      if (src_valid && src_ready && (n_got < c_WORDS)) begin
        if (src_data != test_vec)
          $fatal(1, "gearbox_loop a=%0d b=%0d: mismatch got %h exp %h",
  ...
  task automatic send_word;
    ...
    while (!(snk_valid && snk_ready)) begin
      ...
      guard++;
      if (guard > 1000000)
        $fatal(1, "gearbox_loop a=%0d b=%0d: sink timeout", g_A_WIDTH, g_B_WIDTH);
    end
    @(negedge clk);
    snk_valid = 1'b0;
  endtask
  ...
  initial begin
    ...
    begin
      ...
      while (n_got < c_WORDS) begin
        ...
        guard++;
        if (guard > 2000000)
          $fatal(1, "gearbox_loop a=%0d b=%0d: source timeout got %0d/%0d",
...
module gearbox_loopback_tb;
  ...
  initial begin
    ...
    $display("PASS gearbox_loopback_tb");
    $finish;
  end

  initial begin
    #20ms;
    $fatal(1, "timeout gearbox_loopback_tb");
  end
