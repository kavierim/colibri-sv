...
// Scenario from cc_gearbox_down_tb.vhdl: 10-bit to 8-bit, 8 ns write clock, 5 ns read clock.

`timescale 1ns/1ps

module cc_gearbox_down_tb;
    ...
    if (reset) begin
      ...
      if (src_valid && src_ready && (rd_bits < c_TEST)) begin
        if (src_data != test_vec[c_TEST-1-rd_bits -: c_OUT])
          $fatal(1, "cc_gearbox_down: mismatch at bit %0d got %h exp %h",
  ...
  task automatic send_word(input logic [c_IN-1:0] word);
    ...
    while (!(snk_valid && snk_ready)) begin
      ...
      guard++;
      if (guard > 1000000)
        $fatal(1, "cc_gearbox_down: sink timeout");
    end
    @(negedge snk_clk);
    snk_valid = 1'b0;
  endtask
  ...
  initial begin
    ...
    begin
      ...
      while (rd_bits < c_TEST) begin
        ...
        guard++;
        if (guard > 2000000)
          $fatal(1, "cc_gearbox_down: source timeout at %0d", rd_bits);
      end
    end
    $display("PASS cc_gearbox_down_tb");
    $finish;
  end

  initial begin
    #5ms;
    $fatal(1, "timeout cc_gearbox_down_tb");
  end
