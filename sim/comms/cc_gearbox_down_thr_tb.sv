...
// Scenario from cc_gearbox_down_thr_tb.vhdl: no stalls, in-order data.
// The VHDL check_stable end event compares the bit pointer with TEST_SIZE-1,
// which the pointer (it steps by the output width) never hits, so that check
// does not close. Ready is held high for the whole read.

`timescale 1ns/1ps

module cc_gearbox_down_thr_tb;

  localparam int c_IN = 10;
  localparam int c_OUT = 8;
  localparam int c_BUF = 10;
    ...
    if (reset) begin
      ...
      if (src_valid && src_ready && (rd_bits < c_TEST)) begin
        if (src_data != test_vec[c_TEST-1-rd_bits -: c_OUT])
          $fatal(1, "cc_gearbox_down_thr: mismatch at bit %0d got %h exp %h",
  ...
  task automatic send_word(input logic [c_IN-1:0] word);
    ...
    while (!(snk_valid && snk_ready)) begin
      ...
      guard++;
      if (guard > 1000000)
        $fatal(1, "cc_gearbox_down_thr: sink timeout");
    end
    @(negedge snk_clk);
    snk_valid = 1'b0;
  endtask
  ...
  initial begin
    ...
    begin
      ...
      while (rd_bits < c_TEST) begin
        ...
        guard++;
        if (guard > 2000000)
          $fatal(1, "cc_gearbox_down_thr: source timeout at %0d", rd_bits);
      end
    end
    $display("PASS cc_gearbox_down_thr_tb");
    $finish;
  end

  initial begin
    #5ms;
    $fatal(1, "timeout cc_gearbox_down_thr_tb");
  end
