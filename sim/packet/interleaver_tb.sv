...
// simple: one packet on every input, checked back on that channel.
// interleave_random: g_N_PACKETS hits of every (channel, size) pair, for both
// word interleaving and packet boundaries. Channels are driven together.

`timescale 1ns/1ps

import packet_tb_pkg::*;

module interleaver_case #(
  ...
  parameter int unsigned g_SEED             = 32'hCE11_0001
) (
  input  logic clk_i,
  input  logic reset_i,
  output logic done_o
  ...
  logic [c_CH_W-1:0]       src_channel_o;

  // Fixed memories. Verilator 5.020 miscompiles an array of queues.
  ...
  task automatic sample();
    ...
    for (ch = 0; ch < g_NUM_INPUTS; ch++) begin
      if (c_CH_W'(ch) != src_channel_o)
        continue;
      if (int'(src_empty_o) >= c_WORD_B)
        $fatal(1, "interleaver empty %0d", src_empty_o);
      n_valid = c_WORD_B - int'(src_empty_o);
      if (got_n[ch] == 0 && !src_sop_o)
        $fatal(1, "interleaver ch %0d missing sop words=%0d", ch, g_INTERLEAVE_WORDS);
      for (bi = 0; bi < n_valid; bi++) begin
        gbyte = 8'(be_get(256'(src_data_o), bi, 8, c_WORD_B));
        if (gbyte != exp_mem[ch][exp_rd[ch] + got_n[ch]])
          $fatal(1, "interleaver ch %0d byte %0d got %02x exp %02x data %h words=%0d",
                 ch, got_n[ch], gbyte, exp_mem[ch][exp_rd[ch] + got_n[ch]],
                 src_data_o, g_INTERLEAVE_WORDS);
        got_n[ch]++;
      end
      if (src_eop_o) begin
        if (exp_len_rd[ch] == exp_len_wr[ch])
          $fatal(1, "interleaver ch %0d extra eop", ch);
        elen = exp_len_mem[ch][exp_len_rd[ch]];
        if (got_n[ch] != int'(elen))
          $fatal(1, "interleaver ch %0d len got %0d exp %0d", ch, got_n[ch], elen);
        exp_rd[ch] += int'(elen);
        exp_len_rd[ch]++;
        got_n[ch] = 0;
      end
  ...
  task automatic drive();
    ...
    for (ch = 0; ch < g_NUM_INPUTS; ch++) begin
      ...
      if (!beat[ch] && tx_wr[ch] != tx_rd[ch] && (tx_left[ch] != 0 || tx_len_rd[ch] != tx_len_wr[ch])) begin
        ...
        // Constant index. A variable write of this unpacked port is dropped.
        // Procedural writes. A continuous copy of the unpacked array is
        // scheduled only at reset, so the packed port would stay zero.
        if (ch == 0) begin
          snk_data_i[0] = word;
...
module interleaver_tb;
  ...
  initial begin
    if (3 >= 4)
      $fatal(1, "Cannot instantiate more than 4 sources");
    reset = 1'b1;
    @(posedge clk);
    @(negedge clk);
    reset = 1'b0;
  ...
  initial begin
    #50ms;
    $fatal(1, "interleaver_tb watchdog");
  end

  interleaver_case #(
    .g_INTERLEAVE_WORDS (1'b1),
    .g_SEED             (32'hCE11_0001)
  ) u_words (
    .clk_i   (clk),
    .reset_i (reset),
    .done_o  (done_words)
