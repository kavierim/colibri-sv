...
// Stream buffer. sim/common/stream_buffer_tb.vhdl and run.py:
// 101 bytes in 16..127, 10 ns clock, 50 ns reset, both datapath modes,
// 50% stalls of 1..8 cycles on valid and ready.
  ...
  for (genvar gi = 0; gi < 2; gi++) begin : gen_mode
    ...
    always @(negedge clk) begin
      if (!go) begin
        ...
        // Hold the beat for the clock after a stall.
