...
// Self-check for be_add_lead. Twenty-one packets, shortest 10 bytes, each
// prefixed with the three-byte header 0xBADC0F. snk_sop_i stays 1 outside a
// multi-word packet so the PSL ready assumption holds.

`timescale 1ns/1ps

module be_add_lead_tb;
  ...
  logic [c_DATA_WIDTH-1:0] snk_lead;
  ...
  initial begin : proc_sequencer
    ...
    $display("PASS be_add_lead_tb");
    $finish;
  end

  initial begin : proc_watchdog
    #2ms;
    $fatal(1, "be_add_lead_tb watchdog");
  end
