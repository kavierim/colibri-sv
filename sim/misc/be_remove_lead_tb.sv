...
// Self-check for be_remove_lead. Twenty-one packets carry the three-byte
// header 0xBADC0F plus a payload of 30 down to 10 bytes. The header is
// removed. snk_sop_i stays 1 outside a multi-word packet.

`timescale 1ns/1ps

module be_remove_lead_tb;
  ...
  logic [c_DATA_WIDTH-1:0] src_head;
  ...
  initial begin : proc_sequencer
    ...
    for (int i = 0; i <= c_N_PACKETS; i++) begin
      ...
      for (int b = 0; b < nbytes; b++) begin
        send_b[3 + b] = 8'(c_MAX_LEN - 1 - b);
        expect_b[b]   = 8'(c_MAX_LEN - 1 - b);
      end

      seen = pkts;
      base = cap_n;
      send_pkt(nbytes + 3, send_b);
      expect_pkt(seen, base, nexp, expect_b);
      repeat (10) @(posedge clk);
    end

    $display("PASS be_remove_lead_tb");
    $finish;
  end

  initial begin : proc_watchdog
    #2ms;
    $fatal(1, "be_remove_lead_tb watchdog");
  end
