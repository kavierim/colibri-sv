...
// Self-check for be_add_trail. Twenty-one packets, shortest 10 bytes, each
// suffixed with the three-byte trailer 0xBADC0F. snk_sop_i stays 1 outside a
// multi-word packet so the PSL ready assumption holds.

`timescale 1ns/1ps

module be_add_trail_tb;
  ...
  logic [c_DATA_WIDTH-1:0] snk_trail;
  ...
  initial begin : proc_sequencer
    ...
    for (int i = 0; i <= c_N_PACKETS; i++) begin
      ...
      for (int b = 0; b < nbytes; b++)
        payload[b] = 8'(nbytes - 1 - b);
      for (int b = 0; b < nbytes; b++)
        expect_b[b] = payload[b];
      expect_b[nbytes]     = c_HDR0;
      expect_b[nbytes + 1] = c_HDR1;
      expect_b[nbytes + 2] = c_HDR2;
    ...
    $display("PASS be_add_trail_tb");
    $finish;
  end

  initial begin : proc_watchdog
    #2ms;
    $fatal(1, "be_add_trail_tb watchdog");
  end
