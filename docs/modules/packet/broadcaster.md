...
RTL notes: Broadcasts one Avalon-ST stream to g_NUM_OUTPUTS streams. Each output has its own pipeline buffer, and the input fires only when every buffer is ready.
