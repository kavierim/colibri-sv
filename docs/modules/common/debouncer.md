...
RTL notes: Debouncer. Release log: - 0.1 first release g_RESET_VAL is an unconstrained std_logic_vector in VHDL. data_i and data_o take $bits(g_RESET_VAL); pass a sized vector. g_CLOCK_PERIOD has no VHDL default; 10 ns is only so Verilator can elaborate this module.
