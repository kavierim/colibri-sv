...
RTL notes: Big-endian packet remove-trailing-bytes module. Removes `shl_i` bytes and returns them on `src_tail_o` at `src_eop_o`. `g_REGISTER_OUT` is in the VHDL entity and unused by the architecture.
