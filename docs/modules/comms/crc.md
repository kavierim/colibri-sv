...
RTL notes: Cyclic redundancy check for a packet stream. The CRC is presented with the end-of-packet word.
...
Cyclic redundancy check for a packet stream. The CRC is presented with the end-of-packet word. Stream-facing modules use Avalon-ST records from `colibri_types` unless the RTL exposes a simple valid/ready bus.
