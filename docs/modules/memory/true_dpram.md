...
True dual-port RAM (two write and two read ports).

RTL notes: True dual-port RAM with mixed-width support. The selected model stores the array as logic and updates it in always_ff.
...
True dual-port RAM with mixed-width support. The selected model stores the array as logic and updates it in always_ff. FIFOs support optional first-word fall-through via `g_ENABLE_FWFT`. Packet FIFOs honour Avalon-ST `startofpacket`/`endofpacket`.
...
| --- | --- | --- |
| yes | sim/memory/true_dpram_tb.sv | none |

# Agent notes

- Read `src/memory/true_dpram.sv` before editing; preserve the SPDX header and `` `timescale `` line.
- Match [`CONVENTIONS.md`](../../../CONVENTIONS.md); do not rename ports or packages.
- Run targeted simulation: `sim/memory/true_dpram_tb.sv` when present, or `./verilator/run_all.sh` for full regression.
- Compare behaviour against upstream VHDL at the pinned commit when changing logic.

# Related

- [memory RTL](../../../src/memory/true_dpram.sv)
