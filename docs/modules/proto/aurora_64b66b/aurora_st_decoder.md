...
RTL notes: Aurora 64b/66b Decoder. Simplex multi-lane decoder. snk_link_up_i marks the end of link training and channel bonding. The first packet is dropped. src_error_o is set when the link drops mid-packet, or when a bond word arrives on a single lane. Changelog: - 0.5 Channel Bond lane mismatch error handling - 0.4 code cleanup rewriting - 0.3b update to vhdl style guideline - 0.3 Add Multi-Lane support - 0.2 FIX behaviour when valid is oscillating - 0.1 first release
