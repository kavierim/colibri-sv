...
RTL notes: Aurora 64b/66b Encoder. Simplex multi-lane encoder. A 64-bit Avalon packet stream is encoded into Aurora lanes. A channel bonding word is sent every 64 cycles when more than one lane is used. After reset, idle words are sent for 64 cycles, then snk_ready_o is asserted. changelog: - 0.4 significant rewrite to clean up code (removed fifos) - 0.3b Update to VHDL Style Guideline - 0.3 Update to VHDL common library - 0.2 Adding Channel Bonding Feature - 0.1 first release
