...
SystemVerilog package translated from the upstream VHDL library `colibri`. Import with `import mmap_fifo_csr_pkg::*;` after compiling `src/misc/mmap_fifo/vhdl_if/mmap_fifo_csr_pkg.sv`.
...
Public API is defined in `src/misc/mmap_fifo/vhdl_if/mmap_fifo_csr_pkg.sv`. Use the source file as the authoritative list of types, functions, and constants.
...
- Add `src/misc/mmap_fifo/vhdl_if/mmap_fifo_csr_pkg.sv` via `verilator/colibri.f` or the domain-specific `.f` file that pulls it in.
- VHDL name mapping: see the frozen package table in [`CONVENTIONS.md`](../../CONVENTIONS.md).

# Examples

```systemverilog
import mmap_fifo_csr_pkg::*;
```
...
- Treat `src/misc/mmap_fifo/vhdl_if/mmap_fifo_csr_pkg.sv` as a frozen API surface unless the user explicitly requests a breaking change.
