...
SystemVerilog package translated from the upstream VHDL library `colibri`. Import with `import colibri_mem::*;` after compiling `src/memory/mem_pkg.sv`.
...
Public API is defined in `src/memory/mem_pkg.sv`. Use the source file as the authoritative list of types, functions, and constants.
...
- Add `src/memory/mem_pkg.sv` via `verilator/colibri.f` or the domain-specific `.f` file that pulls it in.
...
- Treat `src/memory/mem_pkg.sv` as a frozen API surface unless the user explicitly requests a breaking change.
