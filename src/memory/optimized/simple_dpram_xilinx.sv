...
// Xilinx-style simple dual-port RAM. Behavioural description for BRAM inference.
// Canonical memory pattern: a logic array, written and read in always_ff.

`timescale 1ns/1ps

module simple_dpram_xilinx #(
  ...
  if (g_A_DATA_WIDTH > g_B_DATA_WIDTH) begin : gen_a_wider
    ...
    always_ff @(posedge wrclk_i) begin : proc_write_ram_mux
      for (int i = 0; i < c_A_RATIO; i++) begin
        if (wren_i)
          memory[int'({wraddr_i, c_A_LSB_W'(i)})] <=
            wrdata_i[(g_A_DATA_WIDTH - 1) - (i * c_MEM_WIDTH) -: c_MEM_WIDTH];
      end
    end
  end else begin : gen_a_direct
    always_ff @(posedge wrclk_i) begin : proc_write_ram_direct
      if (wren_i)
        memory[int'(wraddr_i)] <= wrdata_i;
    end
  end
  ...
  if (g_B_DATA_WIDTH > g_A_DATA_WIDTH) begin : gen_b_wider
      ...
      if (rden_i) begin
        ...
        for (int i = 0; i < c_B_RATIO; i++)
          v_word[(g_B_DATA_WIDTH - 1) - (i * c_MEM_WIDTH) -: c_MEM_WIDTH] =
            memory[int'({rdaddr_i, c_B_LSB_W'(i)})];
        read_b <= v_word;
      end
    end
    ...
    always_ff @(posedge rdclk_i) begin : proc_read_ram_direct
      if (rden_i)
        read_b <= memory[int'(rdaddr_i)];
    end
  end

  if (g_REGISTER_OUT) begin : gen_reg_out
    always_ff @(posedge rdclk_i) begin : proc_reg_out
      rddata_o <= read_b;
    end
  end else begin : gen_comb_out
    assign rddata_o = read_b;
  end
