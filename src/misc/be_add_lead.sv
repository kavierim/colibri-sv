...
// Big-endian packet add-leading-bytes module.
// Shifts an Avalon-ST packet right and inserts `snk_lead_i`.
// `snk_lead_i` and `shl_i` are sampled at `snk_sop_i`.
...
module be_add_lead #(
  ...
  // VHDL concatenations use null slices when the byte count is 0 or the full
  // bus. Shifts below are those concatenations, including the null cases.
  ...
  function automatic logic [g_DATA_WIDTH-1:0] join_lead(
    input logic [g_DATA_WIDTH-1:0] lead_buf,
    input logic [g_DATA_WIDTH-1:0] data,
    input int n
  );
    ...
    return (lead_buf & top_mask) | ((data >> (n * 8)) & ~top_mask);
  endfunction
  ...
  // VHDL field `buf` is `word_buf`: `buf` is a Verilog gate primitive.
  typedef struct {
    fsm_t                        state;
  ...
  logic [g_DATA_WIDTH-1:0] skid_lead;
  logic [g_DATA_WIDTH-1:0] skidb_lead;
  ...
  assign skid_lead = snk_ready_o ? snk_lead_i : skidb_lead;

  always_ff @(posedge clk_i) begin : proc_skid_lead
    if (snk_ready_o && snk_valid_i && !fsm_ready)
      skidb_lead <= snk_lead_i;
  end
    ...
    case (rreg.state)
      S_DATA: begin
        if (v_int.ready && skid.valid) begin
          ...
          if (skid.sop)
            v_int.shl = int'(shl_i);
          // VHDL loop is 0 to g_DATA_WIDTH/8. A larger shl leaves buf unchanged.
          if (v_int.shl <= c_DATA_BYTES) begin
            if (skid.sop)
              v_int.word_buf = msb_bytes(skid_lead, v_int.shl);
            v_int.avst.data = join_lead(v_int.word_buf, skid.data, v_int.shl);
            v_int.word_buf  = lsb_to_msb(skid.data, v_int.shl);
          end
          if (skid.eop) begin
